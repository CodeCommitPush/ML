...
tsla_df = yf.download('TSLA', start='2021-10-01', end='2023-02-02', progress=True)
...
model.add(tf.keras.layers.LSTM(64, input_shape = (Xtrain.shape[1], Xtrain.shape[2])))
model.add(tf.keras.layers.Dense(32, activation = 'relu'))
model.add(tf.keras.layers.Dense(32, activation = 'relu'))
model.add(tf.keras.layers.Dense(1))
model.compile(loss = "mean_squared_error", optimizer = "adam")

model.fit(Xtrain, ytrain, epochs = 200 , validation_data = (Xtest, ytest), batch_size = 16, verbose = 1)
...
at = []
at.append(X_test[len(X_test) - 5 : len(X_test)-2, :X_test.shape[1]])
at = np.array(at)
